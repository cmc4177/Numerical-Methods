...
% Problem 1 Part C
% Numerical Integration Done Using Trapezoidal Rule
%f(a)
i = 1;
fdash(i) = -(x(i)^2-1)*v(i);
pdash(i) = fdash(i)*v(i);
...
for i = 1:1000
    fdash(i+1) = -(x(i+1)^2-1)*v(i+1);
    pdash(i+1) = fdash(i+1)*v(i+1);
    wdash(i) = dt*(pdash(i)+pdash(i+1))/2;
end

pdtotal = (wdash(1000)-wdash(1))*30;